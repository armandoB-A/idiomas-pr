...
<body>
    ...
    <form id="payment-form" action="{{ route('idiomas.comprar') }}" method="POST">
        @csrf
        ...
        <div class="form-container">
            ...
            <div class="field-container">
                ...
                <input id="expiration" name="expiration" type="text" pattern="[0-9/]*" inputmode="numeric"
                    required>
            </div>
            <div class="field-container">
                <label for="cvv">CVV</label>
                <input id="cvv" name="cvv" type="text" pattern="[0-9]*" inputmode="numeric" required>
            </div>
            <input type="hidden" name="idiomas" value="{{ implode(',', $idiomas) }}">
        </div>
    ...
    <script>
        ...
        const cardExpiration = document.getElementById('svgexpire');
        ...
        expirationInput.addEventListener('input', updateCardExpiration);
        cvvInput.addEventListener('input', updateCardCVV);

        // Función para actualizar el nombre en la tarjeta
        function updateCardName(event) {
            cardName.textContent = event.target.value.toUpperCase();
        }

        // Función para actualizar el número de la tarjeta
        function updateCardNumber(event) {
            const cardNumberValue = event.target.value.padEnd(16, '•').match(/.{1,4}/g).join(' ');
            cardNumber.textContent = cardNumberValue;
        }

        // Función para actualizar la fecha de vencimiento en la tarjeta
        function updateCardExpiration(event) {
            const expirationValue = event.target.value.padEnd(4, '•').match(/.{1,2}/g).join('/');
            cardExpiration.textContent = expirationValue;
        }

        // Función para actualizar el CVV en la tarjeta
        function updateCardCVV(event) {
            cardCVV.textContent = event.target.value.padEnd(3, '•');
        }
    </script>
